...
#define brightness 0	//	From 0 (low) to 15
...
void loop()
{
	...
	*/
	...
	if (channel_switch_read > 100 && channel_switch_read < 200) {
		channel_switch = 1;					//	Density Pressed. Was 2 in original.
	};
	if (channel_switch_read > 200 && channel_switch_read < 400) {
		channel_switch = 0;					//	Length Pressed. Was 1 in original.
	};
	if (channel_switch_read > 400) {
		channel_switch = 2;					//	Offset Pressed. Was 0 in original.
	};
	if (channel_switch != 3) {
...
unsigned int euclid(int n, int k, int o) { // inputs: n=total, k=beats, o = offset
	...
	if (per_pulse > 0 && remainder < 2) { // Handle easy cases where there is no or only one remainer
		...
		if (pulses == 0) { pulses = 1; }	//	Prevent crashes when k=0 and n goes from 0 to 1
		int groupa = pulses;
		int groupb = pauses;
		int iteration = 0;
		if (groupb <= 1) {
...
void Sync() {
	...
	for (a = 0; a < channels; a++) {
		...
		if (bitRead(beat_holder[a], read_head) == 1) {
			...
			//lc.setLed(0, 7, 5 - (b * 2), true); // bottom row flash
			if (a == 0) { lc.setLed(0, 7, 5, true); }
			if (a == 1) { lc.setLed(0, 7, 2, true); }
			if (a == 2) { lc.setLed(0, 7, 0, true); }
			pulses_active = true;
			lights_active = true;
		}
